    ...
    <div className="container max-w-2xl pt-24 pb-24">
      ...
      <dl className="grid grid-cols-[auto_1fr] gap-x-6 gap-y-2 mb-10">
        ...
        {typeof user.age === 'number' && (
          <>
            <dt className="text-sm text-muted-foreground">Alder</dt>
            <dd>{user.age}</dd>
          </>
        )}
        {(user.zipCode || user.city) && (
          <>
            <dt className="text-sm text-muted-foreground">Fra</dt>
      ...
      <h2 className="text-xl font-semibold mb-3">Begivenheder du deltager i</h2>
      {attendingDocs.length === 0 ? (
        <p className="text-muted-foreground">Du har endnu ikke tilmeldt dig nogen begivenheder.</p>
      ) : (
      ...
      <h2 className="text-xl font-semibold mt-10 mb-3">Begivenheder du har liket</h2>
      {likedDocs.length === 0 ? (
        <p className="text-muted-foreground">Ingen likede begivenheder endnu.</p>
      ) : (
        <ul className="flex flex-col gap-2">
          {likedDocs.map((e) => {
