    ...
    <footer className="mt-auto border-t border-border bg-black dark:bg-card text-white">
      <div className="container py-14">
            ...
            <div className="flex items-center gap-3 pt-2">
              {socialLinks.map(({ label, href, icon: Icon }) => (
                <a
                  ...
                  className="inline-flex h-9 w-9 items-center justify-center rounded-full border border-white/15 text-white/80 transition hover:border-white hover:text-white"
                >
        ...
        <div className="mt-12 flex flex-col gap-4 border-t border-white/10 pt-6 md:flex-row md:items-center md:justify-between">
          ...
          <nav className="flex flex-wrap items-center gap-x-5 gap-y-2 text-xs text-white/70">
            {navItems.map(({ link }, i) => (
              <CMSLink className="hover:text-white transition" key={i} {...link} />
            ))}
            <ThemeSelector />
          </nav>
        </div>
